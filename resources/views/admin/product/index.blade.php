                    ...
                    <tbody class="divide-y divide-gray-200">
                        @forelse($products as $index => $product)
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4 text-sm text-gray-600">
                                    <span
                                        class="inline-block px-2 py-1 {{ $product->stock > 10 ? 'bg-green-50 text-green-700' : ($product->stock > 0 ? 'bg-amber-50 text-amber-700' : 'bg-red-50 text-red-700') }} rounded text-xs font-semibold">
                                        {{ $product->stock }} Unit
                                    </span>
                                </td>
                                ...
                                <td class="px-6 py-4 text-sm">
                                    <div class="flex items-center justify-center gap-2">
                                        <!-- Edit Button -->
                                        <button
                                            onclick="editProduct({{ $product->id }}, '{{ $product->name }}', '{{ $product->category }}', {{ $product->stock }}, {{ $product->price_per_day }}, '{{ $product->description }}', '{{ $product->status }}')"
            ...
            <form id="productForm" method="POST" action="{{ route('products.store') }}" class="p-6 space-y-5"
                ...
                <div>
                    ...
                    <textarea id="description" name="description"
                        placeholder="Masukkan detail produk, spesifikasi, atau informasi penting lainnya..." rows="5"
